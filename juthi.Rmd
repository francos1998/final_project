...
traffic_cv <- vfold_cv(top5_training,
                       v = 5)
...
top5_mod <- traffic_final %>% 
 ...
 top5_mod$Crossing <- as.factor(top5_mod$Crossing)
top5_mod$Month <- as.numeric(top5_mod$Month)
 top5_mod$Wday <- as.numeric(top5_mod$Wday)
 top5_mod$Hour <- as.numeric(top5_mod$Hour)
  top5_mod$Duration <- as.numeric(top5_mod$Duration)
 top5_mod$Junction <- as.factor(top5_mod$Junction)
 top5_mod$Traffic_Signal <- as.factor(top5_mod$Traffic_Signal)
 

set.seed(327) #for reproducibility

# Randomly assigns 75% of the data to training.
top5_split <- initial_split(top5_mod, 
                             prop = .50)
top5_split

top5_training <- training(top5_split)
top5_testing <- testing(top5_split)

#creating recipe
top5_recipe <- recipe(Status ~ .,
                       data = top5_training) %>% 
  step_mutate(County, 
              County = fct_lump_n(County, n = 5)) %>% 
  step_mutate(City, 
...
top5_lasso_wf <- 
  workflow() %>% 
  add_recipe(top5_recipe) %>% 
  add_model(top5_lasso_mod)

top5_lasso_wf

top5_cv <- vfold_cv(top5_training, v = 3)
penalty_grid <- grid_regular(penalty(),
                             levels = 10)
...
best_param_top5 <- top5_lasso_tune %>% 
  select_best(metric = "accuracy")
best_param_top5

top5_final_lasso <- top5_lasso_wf %>% 
  finalize_workflow(best_param_top5) %>% 
  fit(data = top5_training)
...
# traffic_cv <- vfold_cv(top5_training, v = 5)
# 
